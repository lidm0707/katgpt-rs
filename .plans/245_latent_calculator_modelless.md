...
- [x] 14. **Percent-price operations + float-noise formatting (Plan 245):** added `discount`/`off`/`sale`/`save` (price × (1−pct/100)) and `tax`/`tip`/`vat` (price × (1+pct/100)) via `Computation::PercentPrice` + `PercentDir` enum. `fmt_num` snaps to 9 decimals so `50$ tax 10%` → `result is 55$`.
- [x] 15. **Neuro-symbolic latent-space rewrite (Plan 245):** collapsed 6 logic files into **3**: `tokenizer.rs` (lexical) + `transformer.rs` (the latent-space engine: `embed`→`attend`→`Latent`→`decode`) + `main.rs`. Deleted `lexer/extract/intent/engine/plausibility.rs`; `lib.rs` is thin re-exports. The forward pass (`embed`+`attend`) does NL understanding; `decode` is symbolic arithmetic — neuro-symbolic split. NL operation words now work for **any magnitude** (was single-digit only): `double 15`→30, `20 buy 15`→35. Key fix: NL words go to a separate `nl_op` slot so `TotalCost` (qty+price) wins — spec `I buy 3 at 20$ total` stays `total is 60$`, while `2 buy 1`→3. 17 tests, clippy clean. Zero deps, no flags.
